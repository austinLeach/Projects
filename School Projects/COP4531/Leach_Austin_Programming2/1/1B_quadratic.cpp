...
int quadraticProbing(int number, vector<int> &hashtable, int iteration) {
    int i = (number % size + iteration + iteration*iteration) % size;
    if (i < 0) {
        i *= -1;
    }
    ...
    if (hashtable[i] == -1) {
        ...
        iteration++;
        quadraticProbing(number, hashtable, iteration);
    }
}
...
int main() {
    ...
    for (int i = 0; i < size - size/100; i++) {
        ...
        int probeValue = quadraticProbing(random, hashtable, 0);
        if (i > size/2 && (i % 100) == 0) {
